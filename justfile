...
urls:
    ...
    echo "    Tals (cluster) http://$IP:5179       (the thales-dev namespace in minikube,"
    echo "                                           held open by the user systemd unit"
    echo "                                           thales-portal-forward."
    echo "                                           dev DELIBERATELY: it is the namespace"
    echo "                                           with the seed fixture and a working"
    echo "                                           Keycloak login. thales and"
    echo "                                           thales-pre-prod are the PROD shape -"
    echo "                                           AUTH_MODE=saml with no reachable"
    echo "                                           AD-FS - so they refuse password login"
    echo "                                           by design, and would look broken.)"
    echo ""
    ...
    echo "      ^ REBUILT 2026-08-31 as profile \"thales-scc\" - an OpenShift"
    echo "        restricted-v2 SCC emulation (PSA restricted + Kyverno) running the"
    echo "        thales, thales-dev and thales-pre-prod namespaces. The line above"
    echo "        records the 2026-08-12 deletion, not the state today."
