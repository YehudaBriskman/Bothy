...
| `systemd/bothy-updater.{path,service}` | `/etc/systemd/system/` | The host update executor (`apps/bothy-ops/updater`, build step 4). The path unit watches `~/.local/state/bothy/updates/spool/` - the one directory bothy-ops may write - and the oneshot service drains it under a global lock: re-validate the request against `updates.toml` and a plan it recomputes itself, pre-flight, snapshot to `~/backups/pre-update/`, pull, `just up-<group>`, canaries on response bodies, automatic rollback. It deploys ONLY what the checked-out `main` pins. Enable the **path** unit, not the service: `systemctl enable --now bothy-updater.path`. `just update-status` shows what it did. |
| `systemd/bothy-updater-auto.{timer,service}` | `/etc/systemd/system/` | The automatic channel (`apps/bothy-ops/updater/auto.py`, build step 7). At 03:30, `Persistent=false` (a missed night is skipped, never caught up): inside the `[policy]` window, after tonight's `stacks-backup.service` succeeded, it picks at most ONE `auto` component with a deployable **patch** plan that is not paused and whose container and canaries are green, and writes one request into the spool with the actor `auto` - the executor above runs it unchanged. A rollback or failure pauses auto for that component until an operator clears it (Settings > Updates, or `just update-unpause <component>`). Needs `bothy-updater.path` enabled. Enable the **timer**: `systemctl enable --now bothy-updater-auto.timer`. `just update-auto --dry-run` says what tonight would do. |
| `systemd/bothy-updates-discover.*` | `/etc/systemd/system/` | Every six hours, `apps/bothy-ops/discover_updates.py` compares every pin in `apps/bothy-ops/updates.toml` with the registries, GitHub releases and the helm index, and with what is running - READ-ONLY, it pulls nothing. Writes what Settings > Updates serves (`~/.local/state/bothy/updates/available.json`) and `bothy_update_available{component,level}` for node-exporter's textfile collector. `just updates-discover` runs it by hand. |
| `systemd/wsl-fix-mtu.service`<br>`wsl/sbin/wsl-fix-mtu` | `/etc/systemd/system/`<br>`/usr/local/sbin/` | **Sets eth0's MTU to 1500 at every boot, and the tunnel depends on it.** WSL copies the *smallest* MTU among the Windows host's adapters onto eth0 - Windows' Tailscale adapter is 1280, so eth0 lands on 1280 with no room for WireGuard's ~60 B on top of tailscale0's own 1280. Every full-size packet is then dropped in silence: pages answer `200` with **zero bytes** and Keycloak never answers at all. Enabled; disable it and the box goes half-reachable again after the next WSL restart. See [docs/kb/incidents/2026-08-30](../docs/kb/incidents/2026-08-30-wsl-eth0-mtu-1280.md). |
