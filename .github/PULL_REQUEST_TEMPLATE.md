...
- [ ] **A Postgres MAJOR?** The PR changes, together: `data/postgres/compose.yml`'s image (`<major>.<minor>@sha256:…`), its volume mount to `- postgres<N>_data:` (at `/var/lib/postgresql` for 18+) and the top-level declaration, and `auth/compose.yml`'s keycloak-db-init image (the same ref). Merging it does NOT move the data: `just up-data` refuses until the box runs the guided `postgres-major` plan (Settings > Updates, typed and noted). Expect that night's `upgrade.yml` to fail at the guard - see docs/plans/updates.md, step 8
