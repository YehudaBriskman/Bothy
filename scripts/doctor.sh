...
# worse than one that reports it missing. `bothy-files` took its slot: it is
...
expected="traefik oauth2-proxy keycloak victoriametrics grafana loki alloy cadvisor node-exporter postgres postgres-exporter bothy-web bothy-files bothy-ops bothy-socket-read bothy-socket-write"
