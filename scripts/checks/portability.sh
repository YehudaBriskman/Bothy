...
if [ "${1:-}" = "--update" ]; then
  {
    echo "# Accepted portability hits. Generated by scripts/checks/portability.sh --update."
    echo "#"
    echo "# Every line here is a reason someone else cannot run this repository."
    echo "# The list should only ever get SHORTER. If you are adding to it, you are"
    echo "# adding a reason - say why in the commit message."
    echo "#"
    echo "# file<TAB>kind<TAB>count"
    echo "$now"
  } > "$BASELINE"
