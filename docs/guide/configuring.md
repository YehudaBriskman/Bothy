...
from the web tier's Dockerfile, so `bothy-files` came up as nginx and
...
`bothy-prom.yml` itself is gitignored and generated by
`just bothy-prom-route`, because it carries the basic-auth header the edge
...
**`apps/bothy-files/policy.toml`** governs Bothy Files: which roots exist and
...
| a theme under `apps/bothy-web/data/themes/` | on reload. Anything under `web/src/` needs a rebuild |
