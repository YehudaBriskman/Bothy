...
|---|---|---|
| **~~No spacing scale.~~ Declared and migrated 2026-09-22 (`--sp-*`, design-audit batches 2 and 4).** Was: Padding and gap literals span roughly two dozen distinct values | Every spacing choice is made from scratch, and none of them can be changed globally | Adopt a closed ladder and migrate |
| **~~No type scale.~~ Closed and migrated 2026-09-22 (nine rem steps, `--fs-*`; batches 2 and 4).** Was: Seventeen distinct font sizes, 10.5px to 68px | Same, plus there is no way to check a size is legitimate | Close the list to about nine steps |
