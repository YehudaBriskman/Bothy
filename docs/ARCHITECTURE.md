...
| `bothy-web` `bothy-files` `bothy-ops` `oauth2-proxy` `bothy-socket-read` `bothy-socket-write` `promtail` and every exporter | none | Nothing needs to reach these except Traefik or Prometheus, over `devnet`. |
