...
1. **First question for any "the dev box is broken": is that device on the tailnet?**
   ...
   the tell was a failing name-layer lookup; names are retired, so probe
...
Traefik's `dashboard` router served `api@internal` on `Host(<service>.<base-domain>)`
...
16. **"Dormant" is a decision deferred, and it keeps costing.** The name-layer
