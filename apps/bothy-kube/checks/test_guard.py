...
# Parity with app.py's HANDLERS, read as source rather than imported so this
# file needs nothing app.py imports.
src = open(os.path.join(SVC, "app.py"), encoding="utf-8").read()
start = src.index("HANDLERS = {")
block = src[start:src.index("}", start)]
import re  # noqa: E402
handler_ids = set(re.findall(r'^\s+"([a-z-]+)":\s*h_', block, re.M))
ok(handler_ids == set(CAT), f"app.HANDLERS keys == catalog ids: {sorted(handler_ids)}")
