...
This module is the entire security boundary of the file explorer AND of the
config patcher, so it is pure and separately testable: no HTTP, no filesystem
writes outside the snapshot trash. Everything it returns has already been proven
to sit inside an allowlisted root.

ONE COPY (2026-09). bothy-config used to carry a trimmed copy of this file,
whose header said "the drift risk is real and is handled by a test". The two
tiers are one service now (bothy-files), so the copy is gone and the config
tier's narrower rules are resolve_config() below: resolve() first, exactly as a
file read does, then the [config] section of policy.toml on top. A hole fixed in
resolve() is fixed for both, because there is only one resolve().

It lives in apps/bothy-common/ with the other shared modules, but only
bothy-files imports it - it loads policy.toml at import and fails closed, which
is exactly right there and meaningless in bothy-ops.
...
def _load_policy(path: str) -> dict:
    ...
    _check_snapshot_dir(snaps["path"])

    # [config] - the form patcher's narrower rules. Checked as strictly as the
    # rest, because the config tier's own policy failed closed on every one of
    # these before it was merged in, and a merge is not a reason to relax.
    cfg = doc.get("config")
    if not isinstance(cfg, dict):
        raise PolicyError("policy is missing [config]")
    for key in ("roots", "suffixes", "deny_prefixes", "deny_relpaths"):
        if not isinstance(cfg.get(key), list):
            raise PolicyError(f"policy is missing [config].{key}")
    for r in cfg["roots"]:
        if r not in roots:
            raise PolicyError(f"[config].roots names {r!r}, which is not a declared root")
    # The field allowlist is the whole point of the patcher, so an empty one is
    # a configuration error rather than a quiet no-op: a service that answers
    # every patch with "not a patchable field" is indistinguishable from a
    # broken one at 3am.
    fields = cfg.get("fields")
    if not isinstance(fields, dict) or not fields:
        raise PolicyError("policy declares no [config.fields] - nothing would be patchable")
    ...
    _check_snapshot_dir(csnaps["path"])
    return doc


def _check_snapshot_dir(path: str) -> None:
    ...
    if not os.access(path, os.W_OK | os.X_OK):
        raise PolicyError(
            f"snapshot directory {path} is not writable by uid "
...
@dataclass
class SnapshotNet:
    """One undo net: a trash directory and the policy that prunes it.

    There are TWO, and they stay two after the merge: the file editor's trash
    and the config patcher's config-trash. Sharing one tree would have two write
    paths pruning it under different limits, where the loser is whichever path's
    snapshots the other evicted. Two nets that each work beat one that mostly
    does - and the existing trash directories on disk keep their meaning.
    """
    dir: str
    keep: int
    max_age: int
    # A ceiling on what is worth keeping a copy of. Filling the disk to protect
    # one edit would be a poor trade.
    max_bytes: int


NET = SnapshotNet(
    dir=POLICY["snapshots"]["path"],
    keep=int(POLICY["snapshots"].get("keep", 20)),
    max_age=int(POLICY["snapshots"].get("max_age_days", 30)) * 86400,
    # Any suffix may be written through /write, so this is the only thing between
    # the net and a 200 MB file.
    max_bytes=16 * 1024 * 1024,
)
# A compose file is a page of YAML; 4 MB is already not one.
CONFIG_NET = SnapshotNet(
    dir=POLICY["config"]["snapshots"]["path"],
    keep=int(POLICY["config"]["snapshots"].get("keep", 20)),
    max_age=int(POLICY["config"]["snapshots"].get("max_age_days", 30)) * 86400,
    max_bytes=4 * 1024 * 1024,
)
# The historical names, which the checks and app.py read.
SNAPSHOT_DIR: str = NET.dir
SNAPSHOT_KEEP = NET.keep
SNAPSHOT_MAX_AGE = NET.max_age
SNAPSHOT_MAX_BYTES = NET.max_bytes
...
# ── [config]: what a FORM may patch ─────────────────────────────────────────
#
# Narrower than the editor on every axis, and each narrowing is policy.toml's to
# explain: named roots only, YAML only, edge/dynamic/ never (not even for a
# read), exact-path exclusions, a field allowlist, and a credential-shaped
# filename is a REFUSAL here where the editor only labels it. See
# resolve_config().
_CONFIG = POLICY["config"]
CONFIG_ROOTS = frozenset(_CONFIG["roots"])
CONFIG_SUFFIXES = frozenset(s.lower() for s in _CONFIG["suffixes"])
CONFIG_DENY_PREFIXES = tuple(_CONFIG["deny_prefixes"])
CONFIG_DENY_RELPATHS = frozenset(_CONFIG["deny_relpaths"])
# {name: {kind, max_length?}}. Nothing outside this table is patchable, and
...
def snapshot(root: str, relpath: str, abspath: str,
             ...
             net: SnapshotNet | None = None) -> str | None:
    ...
    """
    # Looked up at CALL time, not bound as a default: the checks repoint
    # SNAPSHOT_DIR at a throwaway directory after import.
    if net is None:
        net = SnapshotNet(SNAPSHOT_DIR, SNAPSHOT_KEEP, SNAPSHOT_MAX_AGE, SNAPSHOT_MAX_BYTES)
    ...
    dest_dir = os.path.realpath(os.path.join(net.dir, root, relpath))
    if not (dest_dir + os.sep).startswith(os.path.realpath(net.dir) + os.sep):
        return None
    ...
    try:
        ...
        _prune(dest_dir, prior, net.keep)
        _sweep(net)
        return path
    except OSError:
        return None
...
def _prune(dest_dir: str, prior: list[str], keep: int) -> None:
    """Hold one file's snapshots to `keep`, oldest dropped first."""
    if len(prior) < keep:
        return
    for old in prior[:len(prior) - keep + 1]:
        try:
...
def _sweep(net: SnapshotNet) -> None:
    ...
    """
    marker = os.path.join(net.dir, ".last-sweep")
    now = time.time()
    try:
    ...
    cutoff = now - net.max_age
    for dirpath, _dirnames, filenames in os.walk(net.dir, topdown=False):
        for fn in filenames:
            if fn == ".last-sweep":
                continue
...
def resolve_config(root_key: str, rel: str, *, for_write: bool = False) -> Resolved:
    """resolve(), then the config patcher's narrower rules. Refuses; never sanitises.

    EVERYTHING resolve() proves still holds - containment on the resolved path,
    the denied components, the per-root top-level rules, no writing through a
    symlink - because resolve() runs first and is the same function a file read
    runs. The rules below only ever take away:

      * the root must be one [config].roots names (the stack repo);
      * a credential-shaped FILENAME is refused outright. The editor only LABELS
        it ([sensitive] explains why a file explorer must still open .env), but
        a form has no reader looking at the file, so there is nobody to warn;
      * [config].deny_relpaths and deny_prefixes apply on READ as well as write.
        They were write-only once and the truth table caught it: a symlink to
        monitoring/prometheus-web.yml (a bcrypt hash) was refused for patching
        and cheerfully readable through /config/fields. All three are checked
        on the RESOLVED path, so a link is judged by where it points;
      * a write needs a writable root AND a YAML suffix. YAML is the only thing
        yamlpatch can splice safely - without it "rename a project" would also
        mean "rewrite a shell script that cron runs".
    """
    if root_key not in CONFIG_ROOTS:
        raise PathRefused(f"unknown root {root_key!r}")
    res = resolve(root_key, rel, for_write=for_write)

    if is_sensitive_name(os.path.basename(res.abspath)):
        raise PathRefused("this file is excluded as a possible secret")

    slashed = res.relpath.replace(os.sep, "/")
    if slashed in CONFIG_DENY_RELPATHS:
        raise PathRefused("this file is excluded by policy")
    if any(slashed.startswith(p) for p in CONFIG_DENY_PREFIXES):
        raise PathRefused(
    ...
    if for_write:
        ...
        if os.path.splitext(res.abspath)[1].lower() not in CONFIG_SUFFIXES:
            raise PathRefused(
                "only " + ", ".join(sorted(CONFIG_SUFFIXES)) + " files may be patched")
    return res
