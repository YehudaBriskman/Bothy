...
is one it created, named `bothy-ops-check-*`, and the guard against slipping
...
TARGET = f"bothy-ops-check-{TAG}"
NET = f"bothy-ops-check-net-{TAG}"
PROXY_READ = f"bothy-ops-check-read-{TAG}"
PROXY_WRITE = f"bothy-ops-check-write-{TAG}"
MADE: list[str] = []
...
def raw(port: int, method: str, path: str, body: bytes | None = None,
        timeout: int = 60) -> int:
    """Straight at a proxy, past bothy-ops. Returns the status code.
...
try:
    ...
    control.DOCKER_READ = f"http://127.0.0.1:{rp}"
    control.DOCKER_WRITE = f"http://127.0.0.1:{wp}"
    control.LOG = AuditLog(os.path.join("/tmp", f"bothy-ops-check-{TAG}.log"))
