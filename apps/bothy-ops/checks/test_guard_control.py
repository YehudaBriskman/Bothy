...
Run: python3 checks/test_guard_control.py
...
for n in ("grafana", "postgres", "bothy-control-check-abc", "a1", "x",
    ...
    allows(guard.valid_container, n, label=f"{n[:24]!r} is a name")
...
for n in ("../etc/passwd",
          ...
          "grafana restart",
          "grafana\nX",
          # THE TRAILING NEWLINE, on its own line because it is the one that
          # PASSED until 2026-09: guard.py used `^...$` with .match(), and `$`
          # matches before a final newline. bothy_common.names uses fullmatch.
          "grafana\n",
          "traefik\n",
          "grafana\ttab",
          "grafana\r\nSet-Cookie: x",
    ...
    refuses(guard.valid_container, n, label=f"name {n[:36]!r} is refused", status=400)

refuses(guard.valid_container, None, label="a null container is refused", status=400)
refuses(guard.valid_container, 42, label="a numeric container is refused", status=400)
refuses(guard.valid_container, ["grafana"], label="a list container is refused", status=400)
refuses(guard.valid_container, {"name": "grafana"}, label="an object container is refused",
        status=400)

print()
print("── the self-severing set ───────────────────────────────────────")

SEVERING_EXPECTED = {"traefik", "bothy-ops", "bothy-socket-read", "bothy-socket-write"}
