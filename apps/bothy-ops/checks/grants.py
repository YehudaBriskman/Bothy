...
── what it asserts ─────────────────────────────────────────────────────────
  ...
  socket         mounted :ro on both, and NOT mounted into bothy-ops at all
...
#
# ONE exception, and it is named rather than tolerated: SYSTEM=1 on the READ
# proxy. bothy-socket-read is also the portal's Docker API since 2026-09, and the
# Data & disk card reads /system/df through it (the edge routes that one exact
# Path and nothing else under /system). It is read-only under POST=0, carries no
# Env, and it must never appear on the WRITE proxy.
READ_EXTRA = {"SYSTEM"}
for name, e in ((READ, r), (WRITE, w)):
    for fam in FAMILIES:
        if fam == "CONTAINERS":
        ...
        if name == READ and fam in READ_EXTRA:
            ok(e.get(fam) == "1", f"{name} {fam}=1 - /system/df for the portal, read-only under POST=0")
        else:
...
for name in (READ, WRITE):
    ...
    nets = re.search(r"^    networks: \[([^\]]*)\]", b, re.M)
    got = sorted(n.strip() for n in nets.group(1).split(",")) if nets else None
    # WHO can reach each proxy is the grant as much as its env is. The write
    # proxy meets bothy-ops and nothing else; the read proxy also meets Traefik
    # (socketnet), which is safe only because it is POST=0 - asserted above.
    want = ["controlsocknet", "socketnet"] if name == READ else ["controlsocknet"]
    ok(got == want, f"{name} is on exactly {want} - never opsnet, never devnet ({got})")

svc = OPS.block(SVC_NAME)
ok("docker.sock" not in svc,
   "bothy-ops mounts NO docker socket - it reaches the daemon only through "
   "the two proxies")
...
ok(bool(_user), f"bothy-ops declares a user ({_user.group(1) if _user else None!r})")
if _user:
    spec = _user.group(1)
    ok(spec == '${PUID:-1000}:${PGID:-1000}',
       f"bothy-ops' uid is configurable but pinned by default ({spec})")
    _default = re.match(r'^\$\{PUID:-(\d+)\}', spec)
    ok(bool(_default) and _default.group(1) != "0",
       "bothy-ops does not run as root by default")
ok("read_only: true" in svc, "bothy-ops' own filesystem is read-only")
...
for service in (SVC_NAME, READ, WRITE):
    cn = OPS.container_name(service) if service == SVC_NAME else container_name(service)
...
# repo keeps rediscovering. The floor is the two proxies that exist today,
# bothy-socket-read and bothy-socket-write (three until 2026-09, when the
# portal's read-only proxy merged into bothy-socket-read). Fewer means the walk,
# the extension filter or the image match stopped working - not that the box got
# safer.
ok(len(found) >= 2,
