...
Run: python3 checks/config_http.py

Needs nothing running. It starts the REAL bothy-files handler from app.py on a loopback port
...
# The shipped policy, with only the container paths rewritten. Since the config
# tier merged into bothy-files (2026-09) that policy also declares the editor's
# other roots and trash; they get empty throwaway directories, because the
# loader refuses a root that is not a directory - and nothing here reaches them.
...
for want, got in (('path = "/repos/stacks"', ROOT),
    ...
    if want not in POLICY_SRC:
        sys.exit(f"checks/config_http.py cannot find {want!r} in policy.toml - the "
                 f"check rewrites the container paths and has gone out of step with it")
...
os.environ["CONFIG_AUDIT_LOG"] = AUDIT
os.environ["AUDIT_LOG"] = os.path.join(TMP, "audit", "writes.log")
os.environ["PORT"] = "0"
