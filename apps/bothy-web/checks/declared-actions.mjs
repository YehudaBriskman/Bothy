...
import { verbsFor, consequenceOf } from './actions-mod.mjs';
...
for (const name of ['bothy-web', 'bothy-files', 'portal-next', 'portal-files',
  ...
  check(`start ${name} does not warn`, consequenceOf(name, 'start').selfAffecting, false);
}
check('bothy-web says it takes this page down',
  /takes this page down/.test(consequenceOf('bothy-web', 'stop').warning ?? ''), true);
check('an unrelated container carries no warning',
  consequenceOf('grafana', 'stop').selfAffecting, false);
