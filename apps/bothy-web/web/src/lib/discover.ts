...
// portal-next / portal-files are the pre-2026-09 names of bothy-web / bothy-files.
const INFRA_PROJECTS = new Set(['edge', 'bothy', 'portal', 'portal-next', 'portal-files', 'bothy-web', 'bothy-files']);
...
// (bothy-web-fallback has no Host at all), HostRegexp, multi-host Host(`a`,`b`).
...
// `- ../..:/repos/stacks:rw` in apps/bothy-files/compose.yml - and compose
...
export function stateToStatus(state: string, statusText?: string): Status {
  switch (state) {
    ...
    case 'exited': {
      ...
      // bothy-collector: that also means an OOM kill, and /containers/json
...
export function merge(
  ...
  for (const r of routers) {
    ...
    if (!host) continue; // Routes tab only (bothy-web-fallback)
